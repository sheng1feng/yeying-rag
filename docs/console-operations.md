# Knowledge 控制台操作手册

本文档面向测试、运营与交付同学，说明如何通过前台完成知识库联调与排查。

## 1. 登录与准备

1. 打开控制台首页
2. 点击“钱包登录”
3. 登录成功后，先确认“系统就绪状态”
4. 在“资产仓库”页配置一把写凭证
5. 在“知识库”页导入一把或多把读凭证

## 2. 创建知识库

1. 进入“知识库”
2. 输入名称、描述和基础检索参数
3. 点击“创建知识库”
4. 选中目标知识库作为当前工作对象
5. 若后续修改 `chunk_size` / `chunk_overlap` / `embedding_model`，系统会自动为已有文档触发重建，并对现有 source 重新扫描后重建 evidence

## 3. 上传与绑定资产

1. 进入“资产仓库”
2. 如果还没有任何可用 `ak/sk`，先使用“初始化 uploads 读写凭证（推荐）”或“只初始化 app 根写凭证（高级）”
3. 选择浏览凭证，确认当前正在查看的目录
4. 使用写凭证把文件上传到 `Knowledge App` 的 `uploads/` 目录或其他受支持目录
5. 上传成功后可点击“定位”；如果当前已经选中读凭证，也可直接点击“导入”
6. 如果当前正在用写凭证浏览，想导入刚上传的文件，需要先切回读凭证，或先在“知识库”页创建带读凭证的绑定
7. 如果要长期纳入当前知识库，在“知识库”页选择读凭证并绑定源路径

说明：

- 临时初始化流程由浏览器完成 challenge 签名，再由 `knowledge` 后端代理完成 key 创建、绑定、目录创建与本地凭证回填
- 如果 bootstrap 返回 `manual_cleanup_required`，控制台会提供一次补偿清理入口，用于撤销本次生成的远端 key
- 上传只依赖写凭证
- 绑定源依赖读凭证
- browse / preview / 手工任务里显式传入的 `credential_id` 现在都只接受读凭证
- 绑定路径必须位于当前 app 目录内，并落在所选读凭证的 `root_path` 范围内

## 4. 导入与任务治理

1. 进入“导入任务”
2. 填写目标知识库与源路径；如是手工任务，可按需显式指定读凭证
3. 创建导入 / 重建 / 删除任务
4. 在任务列表中查看状态
5. 点击“详情”查看 task item 成功、跳过、失败明细
6. 若失败，可使用“重试”
7. 当源路径是目录时，删除任务会递归清理该目录下已索引的所有文档，而不再只删除同名精确路径
8. 若知识库已经配置绑定源，优先使用“导入全部绑定源 / 重建全部绑定源 / 删除全部绑定源索引”，减少手动维护路径
9. 在“知识库工作台”里查看每个绑定源的 `pending_sync / syncing / indexed / failed / disabled` 状态，并决定下一步导入或重建动作
10. worker 现在使用数据库租约协调运行状态；如果实例异常退出，租约超时后其他实例可继续接管待处理任务

说明：

- 手工任务里的 `credential_id` 必须是读凭证
- 如果不想手工选择读凭证，优先先创建 binding，再用“按绑定源创建任务”

## 5. 核验证据与知识项

1. 进入“文档与切片”
2. 查看导入后的文档列表
3. 选中文档查看 chunk 详情
4. 再通过 `evidence` 与 `items` API 核验证据、候选知识项和正式知识项是否一致

## 6. 发布、授权与检索审计

1. 先通过 `releases` 发布当前工作区
2. 通过 `grants` 为服务创建授权
3. 用 `POST /service/search*` 验证服务读面
4. 用 `search-lab/compare` 对比 `formal_only / evidence_only / formal_first`
5. 用 `retrieval-logs` 审计服务查询
6. 用 `source-governance` 查看 `source_missing / stale` 治理状态

## 7. 运维排查

优先查看以下页面：

- “系统就绪状态”
- “运维状态”中的运行概览
- 存储与模型健康
- Worker 状态
- 最近失败任务

说明：`/ops/*` 运维接口需要先登录 `knowledge`，不再对匿名访问开放。

建议的审计排查顺序：

1. 先确认服务当前命中的 release 与 grant 选择策略
2. 再查 `retrieval-logs` 看 query、mode、result_summary 与 trace
3. 如果命中异常，用 `search-lab/compare` 对比 formal/evidence 三路结果
4. 如果怀疑来源问题，再查 `source-governance`
5. 如果怀疑任务或索引异常，再结合 `GET /ops/tasks/failures` 与运维状态排查

## 8. 当前产品边界

- `knowledge` 不回写 chunk、embedding 或知识处理结果到 `warehouse`
- 在配置写凭证和上传文件时，`knowledge` 可能会在写凭证权限范围内创建当前 app 目录或目标上传目录
- 当前不启用 rerank
- `memory` 仍保留为兼容模块，但不再是控制台主流程
- `warehouse` 仍是唯一资产中心
- 旧 `/warehouse/auth/*` 绑定接口已经从当前仓库删除，控制台主流程是“bootstrap 初始化或手工导入读/写凭证，再浏览 / 上传 / 绑定 / 导入”
- `revoke-local` 会保留本地凭证记录，但后续 browse / upload / task 不会继续使用被本地吊销的凭证
