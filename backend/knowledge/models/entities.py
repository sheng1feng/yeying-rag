...
class WarehouseAccessCredential(Base):
    ...
    credential_source: Mapped[str] = mapped_column(String(32), default="manual_import", nullable=False)
    upstream_access_key_id: Mapped[Optional[str]] = mapped_column(String(128), nullable=True)
    provisioning_attempt_id: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    provisioning_mode: Mapped[Optional[str]] = mapped_column(String(32), nullable=True)
    remote_name: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
